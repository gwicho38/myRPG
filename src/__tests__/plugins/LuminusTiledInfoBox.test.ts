/**
 * Tests for LuminusTiledInfoBox plugin
 */
...
// Mock LuminusDialogBox
jest.mock('../../plugins/LuminusDialogBox', () => ({
	LuminusDialogBox: jest.fn().mockImplementation(() => ({
		create: jest.fn(),
		canShowDialog: true,
...
describe('LuminusTiledInfoBox', () => {
	let infoBox: LuminusTiledInfoBox;
	let mockScene: any;
	let mockUiScene: any;
	let mockPlayer: any;
	let mockMap: any;
	...
	describe('Constructor', () => {
		it('should initialize with scene, player, map, and uiScene', () => {
			infoBox = new LuminusTiledInfoBox(mockScene, mockPlayer, mockMap, mockUiScene);
		...
		it('should create LuminusDialogBox with uiScene and player', () => {
			infoBox = new LuminusTiledInfoBox(mockScene, mockPlayer, mockMap, mockUiScene);

			const { LuminusDialogBox } = jest.requireMock('../../plugins/LuminusDialogBox');
			expect(LuminusDialogBox).toHaveBeenCalledWith(mockUiScene, mockPlayer);
		});
		...
		it('should store luminusDialogBox instance', () => {
			infoBox = new LuminusTiledInfoBox(mockScene, mockPlayer, mockMap, mockUiScene);

			expect(infoBox.luminusDialogBox).toBeDefined();
		});
	});
	...
	describe('create()', () => {
		...
		it('should call luminusDialogBox.create()', () => {
			infoBox.create();

			expect(infoBox.luminusDialogBox.create).toHaveBeenCalled();
		});
	...
	describe('Overlap Callback', () => {
		...
		it('should set allProperties on overlap', () => {
			...
			expect((infoBox.luminusDialogBox as any).allProperties).toEqual(mockZoneWithChat.properties);
		});
		...
		it('should set isOverlapingChat to true', () => {
			...
			expect(infoBox.luminusDialogBox.isOverlapingChat).toBe(true);
		});
		...
		it('should make action button visible', () => {
			...
			expect(infoBox.luminusDialogBox.actionButton.visible).toBe(true);
		});
		...
		it('should make interaction icon visible', () => {
			...
			expect(infoBox.luminusDialogBox.interactionIcon.visible).toBe(true);
		});
		...
		it('should position interaction icon above player', () => {
			...
			expect(infoBox.luminusDialogBox.interactionIcon.setPosition).toHaveBeenCalledWith(
		...
		it('should set dialog chat content', () => {
			...
			expect(infoBox.luminusDialogBox.chat).toEqual(['Hello!', 'Welcome!']);
		});
	...
	describe('Process Callback', () => {
		...
		it('should return canShowDialog value when true', () => {
			infoBox.luminusDialogBox.canShowDialog = true;
		...
		it('should return canShowDialog value when false', () => {
			infoBox.luminusDialogBox.canShowDialog = false;
	...
	describe('Integration', () => {
		it('should handle complete workflow: create zones, overlap, show dialog', () => {
			...
			expect(infoBox.luminusDialogBox.isOverlapingChat).toBe(true);
			expect(infoBox.luminusDialogBox.actionButton.visible).toBe(true);
			expect(infoBox.luminusDialogBox.chat).toEqual(['Test message']);
		});
