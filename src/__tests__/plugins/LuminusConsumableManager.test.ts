...
describe('LuminusConsumableManager', () => {
	let manager: LuminusConsumableManager;
	let mockScene: any;
	let mockPlayer: any;
	let mockItem: any;
	...
	beforeEach(() => {
		...
		manager = new LuminusConsumableManager();
	});

	describe('Constructor', () => {
		it('should initialize with null luminusEntityTextDisplay', () => {
			expect(manager.luminusEntityTextDisplay).toBeNull();
		});
	});
	...
	describe('recover', () => {
		...
		it('should update HUD progress bar if it exists', () => {
			...
			expect(mockPlayer.luminusHUDProgressBar.updateHealth).toHaveBeenCalledWith(mockPlayer.attributes.health);
		});

		it('should not update HUD progress bar if it does not exist', () => {
			mockPlayer.luminusHUDProgressBar = null;
