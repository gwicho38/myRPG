...
const getSceneWithDialog = () => {
	...
	let targetScene = scenes.find((s) => s.neverquestDialogBox);
	...
	if (!targetScene) {
		console.error(
			'No scene with dialog system found! Active scenes:',
...
const debugDialog = () => {
	...
	let dialogBox = scene.neverquestDialogBox;
	if (!dialogBox && scene.neverquestTiledInfoBox) {
		dialogBox = scene.neverquestTiledInfoBox.neverquestDialogBox;
	}
...
const showTestDialog = () => {
	...
	if (!dialogBox) {
		...
		console.log(
			...
			Object.keys(scene).filter((k) => k.includes('dialog') || k.includes('Dialog'))
		);
		return;
	}
...
const findAllDialogObjects = () => {
	...
	scenes.forEach((scene) => {
		...
		const dialogProps = Object.keys(scene).filter(
			(key) =>
				key.toLowerCase().includes('dialog') ||
				key.toLowerCase().includes('chat') ||
				key.toLowerCase().includes('neverquest')
		);
		...
		if (scene.children) {
			const dialogChildren = scene.children.list.filter(
				(child) => child.name && (child.name.includes('dialog') || child.name.includes('chat'))
			);
